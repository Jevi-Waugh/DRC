...
def main():
    ...
    while True:
        ...
        message = write_message(centroid, droid.obstacle, droid.arrow, droid.start_line, droid.end_line)
...
def write_message(c, obs, arrow, start_line, end_line):
    iscent = False if c == (None, None) else True
    x = -1 if None else c[0]
    y = -1 if None else c[1]
    return struct.pack("iiiiiiiii", SIGNITURE, iscent, x, y, obs, arrow, start_line, end_line)
